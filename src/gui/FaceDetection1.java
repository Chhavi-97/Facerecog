...
public class FaceDetection1 extends javax.swing.JFrame {
    ...
    CascadeClassifier faceDetector = new CascadeClassifier(FaceDetection.class.getResource("cascade.xml").getPath().substring(1));
                ...
                while (runnable) {
                    if (webSource.grab()) {
                        try {
                            ...
                            for (Rect rect : faceDetections.toArray()) {
                                // System.out.println("ttt");
                                Imgproc.putText(frame,"Student", new Point(rect.x, rect.y), Core.FONT_HERSHEY_SIMPLEX,1,new Scalar(0,255,0),2);//adding text to bounding box
                                Imgproc.rectangle(frame, new Point(rect.x, rect.y), new Point(rect.x + rect.width, rect.y + rect.height),
                                        new Scalar(0, 255, 0));
                            }
                            Imgcodecs.imencode(".bmp", frame, mem);
